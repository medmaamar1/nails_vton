...
--------------------------
Usage:
    python train.py --data_root /path/to/dataset --epochs 100 --batch_size 32
"""
...
def train_one_epoch(model, loader, optimizer, criterion, scaler, device, use_amp):
    ...
    for i, batch in enumerate(loader):
        ...
        total_dir_loss += loss_dict.get('l1_dir', loss_dict.get('l2_dir', 0.0))
        ...
        if (i + 1) % 50 == 0:
            ...
            # RAM/VRAM Lockdown: Frequent flush to prevent pile-up
...
def main():
    ...
    if args.resume and Path(args.resume).exists():
        ...
        print(f"Resumed from epoch {start_epoch} (best IoU={best_val_bin_iou:.4f})")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        t0 = time.time()
        print(f"\n{'='*65}")
        print(f"Epoch {epoch+1}/{args.epochs} LR=[enc={current_lrs[0]}, dec={current_lrs[1]}]")
        ...
        print(f"Epoch {epoch+1} — {elapsed:.0f}s | "
              f"train loss={train_loss:.4f}  bin_iou={train_bin_iou:.4f}  | "
              f"val loss={val_loss:.4f}  val_bin_iou={val_bin_iou:.4f}")
        ...
        if val_bin_iou > best_val_bin_iou:
            ...
            print(f"  ✓ New best val IoU: {best_val_bin_iou:.4f}")
    ...
    print(f"\nTraining complete. Best val IoU: {best_val_bin_iou:.4f}")
